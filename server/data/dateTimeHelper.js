...
factory.getFriendlyDate = function (date) {
    ...
    return {
        dayAsString: this.getRoDay(date.getDay()),
        dayAsShortString: this.getRoShortDay(date.getDay()),
        dayOfMonth: dd,
        monthAsString: this.getRoMonth(m - 1),
        monthAsShortString: this.getRoShortMonth(m - 1),
        year: yyyy,
        ymd: yyyy + '-' + mm + '-' + dd,
        dmy: dd + '.' + mm + '.' + yyyy // 23.07.2015
    };
};
factory.getDateFromString = function (date) {
...
factory.getStringFromString = function (dateStr) {
...
factory.getStringFromDate = function (date) {
    return this.getFriendlyDate(date).ymd;
};
factory.getRoToday = function () {
    var utcDate = new Date();
    var roDate = new Date(utcDate);
    // roDate.setHours(utcDate.getHours() + config.roUtcOffset); // Ro time
    roDate.setHours(utcDate.getHours() + 3); // TODO: fix that
    return roDate;
};
factory.getRoTodayStr = function () {
