...
module.exports = function (app) {
    var env = app.get('env');
    app.set('views', environment_1.config.root + '/server/views');
    app.engine('.hbs', exphbs({
        ...
        layoutsDir: environment_1.config.root + '/server/views/layouts/',
        partialsDir: environment_1.config.root + '/server/views/partials/',
    ...
    app.locals.gaCode = environment_1.config.gaCode; // google Analytics code (e.g. 'UA-72165579-1'); http://stackoverflow.com/a/25097453
    if (env === 'production' || env === 'staging') {
        app.use(favicon(path.join(environment_1.config.root, 'client', 'favicon.ico')));
        app.use(express.static(path.join(environment_1.config.root, 'client'), { index: '_' }));
        app.set('appPath', path.join(environment_1.config.root, 'client'));
    }
    else {
        app.use(favicon(path.join(environment_1.config.root, 'client', 'favicon.ico')));
        ...
        app.use('/', express.static(path.join(environment_1.config.root, 'client'), { index: '_' }));
        app.use('/node_modules', express.static(path.join(environment_1.config.root, 'node_modules'))); // to serve node_modules
        app.set('appPath', path.join(environment_1.config.root, 'client'));
    }
    ...
    app.use('/public', express.static(path.join(environment_1.config.root, 'server/public')));
    // for js files used by some views
    app.use('/views', express.static(path.join(environment_1.config.root, 'server/views')));
